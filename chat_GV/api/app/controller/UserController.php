...
class UserController{
    ...
    public function getUserByIdGV($unique_id){
        $sql = mysqli_query($this->conn->connect(), "SELECT * FROM `taikhoan` JOIN giaovien ON giaovien.TenDN = taikhoan.TenDN WHERE taikhoan.TenDN = '{$unique_id}'");
        if(mysqli_num_rows($sql)>0){
            return mysqli_fetch_assoc($sql);
        }
    }
    ...
    public function getDatatoHS($magv){
    ...
    { 
        ...
        ORDER BY msg_id DESC LIMIT 1";
            ...
            // content
            $rs .= '<a href="GV.php?hoithoai='.$row['MaHS'].'">
                  <div class="content">
    ...
    public function getFriendListtoGV($query): string
    {
        ...
        ORDER BY msg_id DESC LIMIT 1";
            ...
            // content
            $rs .= '<a href="GV.php?hoithoai='.$row['MaQL'].'">
                  <div class="content">
                    <img src="view/images/'.$row['HinhAnh'].'"/>
                    <div class="details">
                      <span>'.$row['TenQL'].' (';
